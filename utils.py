...
def eval_results(run_file_path,
        ...
        qrels="./data/msmarco-doc/msmarco-docdev-qrels.tsv" ):
...
def read_qdr_pairs(QD_FILE_PATH):
    qds = {}
    with open(QD_FILE_PATH, 'r', encoding='utf-8') as qdsf:
        for line in qdsf:
            ss = line.strip().split('\t')
...
def read_qd_from_mz(QD_MZ_FILE_PATH):
    qd = {}
    with open(QD_MZ_FILE_PATH, 'r', encoding='utf-8') as qdsf:
        for line in qdsf:
            ss = line.strip().split('\t')
            qid = int(ss[0])
            did = int(ss[1][1:])
...
def read_qd_from_mz_passage(QD_MZ_FILE_PATH):
    qd = {}
    with open(QD_MZ_FILE_PATH, 'r', encoding='utf-8') as qdsf:
        for line in qdsf:
            ss = line.strip().split('\t')
